...
import { MainTicketsTable, CollabTicketsTable, PendingTicketsTable, TeamMetricsTable } from './TicketTables';
...
const Dashboard: React.FC<DashboardProps> = ({ dailyData, historicalData, allMainTickets, allPendingTickets, allCollabTickets, onUpdateTicket, onJumpToDate, availableDates }) => {
  ...
  const kpiData = useMemo(() => {
    ...
    return { totalToday, pendingToday, closedToday, avgTimeSpent, slaRate, avgHoursOpen };
  ...
  const smartMetrics = useMemo(() => {
    ...
    const allTkts = [...(dailyData.mainTickets || []), ...(dailyData.collabTickets || [])];
  ...
  const filteredTickets = useMemo(() => {
    const getSrc = <T extends AnyTicket>(all: T[], daily: T[]) => {
      ...
      const start = new Date(new Date(dailyData.date.split('/').reverse().join('-')));
      start.setDate(start.getDate() - 7);
      return all.filter(t => new Date(t.createdOn) >= start);
    };
  ...
  }, [dailyData, allMainTickets, allCollabTickets, allPendingTickets, statusFilter, priorityFilter, searchTerm, effectiveTimeRange]);

  const chartHistory = useMemo(() => {
    if (effectiveTimeRange === 'all') return historicalData;
    const end = new Date(dailyData.date.split('/').reverse().join('-'));
    const start = new Date(end);
    if (effectiveTimeRange === 'week') start.setDate(end.getDate() - 7);
    return historicalData.filter(t => {
        const d = new Date(t.createdOn);
        return effectiveTimeRange === 'day' ? d.toDateString() === end.toDateString() : (d >= start && d <= end);
    });
      ...
      <section className="space-y-4">
        ...
        {effectiveTimeRange !== 'day' && (
            <div className="bg-brand-950/40 border border-brand-800/50 text-brand-300 px-6 py-3 rounded-2xl text-[11px] font-bold uppercase tracking-widest flex items-center justify-center animate-in fade-in shadow-xl backdrop-blur-sm">
                ...
                Context: {effectiveTimeRange === 'all' ? "Deep Historical Audit" : "Rolling 7-Day Performance Window"}
            </div>
        )}
      </section>
