...
import type { DailyDataCollection, HistoricalTicket, MainTicket, PendingTicket, CollabTicket, AnyTicket } from '../types';
...
export const useTicketData = () => {
  ...
  const [allTickets, setAllTickets] = useState<{ main: MainTicket[], pending: PendingTicket[], collab: CollabTicket[] }>({ main: [], pending: [], collab: [] });
  ...
  const fetchData = useCallback(() => {
    ...
    try {
        ...
        const gMain: MainTicket[] = [], gPending: PendingTicket[] = [], gCollab: CollabTicket[] = [];
        ...
        parsedData.forEach(ticket => {
            ...
            if (!collection[dateKey]) collection[dateKey] = { date: formatted, mainTickets: [], collabTickets: [], pendingTickets: [], techTeamMetrics: [], upcomingProjects: [] };
            ...
            if (!dayMap.has(ticket.ticketIDsSequence)) {
                ...
                if (isPending) { collection[dateKey].pendingTickets.push(common); gPending.push(common); }
                else { collection[dateKey].mainTickets.push(common); gMain.push(common); }
        ...
        Object.keys(collection).forEach(dk => {
            ...
            [...day.mainTickets, ...day.pendingTickets, ...day.collabTickets].forEach(t => {
        ...
        setAllTickets({ main: gMain, pending: gPending, collab: gCollab });
